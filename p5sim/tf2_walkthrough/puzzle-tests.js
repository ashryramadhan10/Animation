  ...
  test("algo catalog lists its sections in site order and validates", () => {
    withCatalog(algoApi(), () => {
      ...
      puzzleList().forEach((puzzle, index) => {
        ...
        const singleOperation = ["chessboard-and-queens", "lower-bound", "upper-bound", "fenwick-add", "fenwick-prefix", "fenwick-kth", "heap-push", "heap-pop", "segment-tree-update", "segment-tree-query", "subarray-node-merge", "kth-ancestor", "dsu-find", "dsu-union", "knights-tour", "coin-piles", "tower-of-hanoi", "creating-strings", "apple-division", "raab-game-i", "mex-grid-construction", "grid-path-description"].includes(puzzle.id);
