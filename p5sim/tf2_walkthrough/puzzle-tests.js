(function runPuzzleTests(root) {
  ...
  test("algo catalog lists its sections in site order and validates", () => {
    withCatalog(algoApi(), () => {
      ...
      assert(status.valid, status.message);
      const api = puzzlesApi();
      same(api.TF2_PUZZLE_TRACKS.map((track) => track.id), ["intro", "sorting", "dp", "graphs", "range", "trees", "math", "strings"]);
      ...
      puzzleList().forEach((puzzle, index) => {
        ...
        const singleOperation = ["chessboard-and-queens", "lower-bound", "upper-bound", "fenwick-add", "fenwick-prefix", "fenwick-kth", "heap-push", "heap-pop", "segment-tree-update", "segment-tree-query", "subarray-node-merge", "kth-ancestor", "tree-lca", "mul-mod", "mod-pow", "mod-inverse", "choose", "factorize", "matrix-multiply", "dsu-find", "dsu-union", "knights-tour", "coin-piles", "tower-of-hanoi", "creating-strings", "apple-division", "raab-game-i", "mex-grid-construction", "grid-path-description"].includes(puzzle.id);
      ...
      same(Array.from(unlocked), ["weird-algorithm", "distinct-numbers", "dice-combinations", "counting-rooms", "static-range-sum-queries", "subordinates", "josephus-queries", "word-combinations"]);
    });
  });
