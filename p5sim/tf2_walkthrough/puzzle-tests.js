(function runPuzzleTests(root) {
  ...
  test("algo catalog lists its sections in site order and validates", () => {
    withCatalog(algoApi(), () => {
      ...
      assert(status.valid, status.message);
      const api = puzzlesApi();
      same(api.TF2_PUZZLE_TRACKS.map((track) => track.id), ["intro", "sorting", "dp", "graphs", "range", "trees", "math", "strings", "geometry", "window", "bitwise", "construction", "advanced", "advanced-graphs", "counting", "additional-i"]);
      ...
      same(Array.from(unlocked), ["weird-algorithm", "distinct-numbers", "dice-combinations", "counting-rooms", "static-range-sum-queries", "subordinates", "josephus-queries", "word-combinations", "cross-sign", "generated-array", "counting-bits", "inverse-inversions", "subset-sums", "nearest-shops", "filled-subgrid-count-i", "shortest-subsequence"]);
    });
  });
