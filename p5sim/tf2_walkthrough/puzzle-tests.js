(function runPuzzleTests(root) {
  ...
  test("algo catalog lists its sections in site order and validates", () => {
    withCatalog(algoApi(), () => {
      ...
      assert(status.valid, status.message);
      const api = puzzlesApi();
      same(api.TF2_PUZZLE_TRACKS.map((track) => track.id), ["intro", "sorting", "dp", "graphs", "range", "trees", "math", "strings", "geometry", "window", "bitwise", "construction"]);
