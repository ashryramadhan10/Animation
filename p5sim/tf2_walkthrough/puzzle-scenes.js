(function exposePuzzleScenes(root) {
  ...
  const algoScene = {
    fixtures: {
      ...
      ballTest: (values) => [[Math.round(values.n), Math.round(values.m), String(Math.round(values.k))]],
      ballGrid: (values) => [[Math.round(values.n), Math.round(values.m)]],
      pointQuery: (values) => [[Math.round(values.x), Math.round(values.y)]],
