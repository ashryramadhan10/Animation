# Algo Lab Phase 1: Introductory Problems Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Complete the Introductory Problems section of the Algo Lab: all 24 CSES tasks in site order, each with the official sample as its first case, named mistakes, hidden cases where the task has a size, and brute-force or validity checks in the tests.

**Architecture:** `algo/01-introductory.js` is rewritten in full (the six existing puzzles keep their code and move into site order). Two generic scene views are added: `letter-grid` and `number-grid`. The stress harness in the tests gains validity checks (`check(args, output)`).
...
In `puzzle-tests.js`, replace the section-id line for intro with:
...
Replace the `singleOperation` list line with:
...
Replace the stress block (from `      puzzleList().forEach((puzzle) => {` through `        assert(next, "catalog present");\n      });`) with:
...
In `puzzle-scenes.js`, insert directly before `      } else if (view === "graph") {`:
...
- [ ] **Step 3: The section file**

Replace `p5sim/tf2_walkthrough/algo/01-introductory.js` with:
...
Add these fixtures to `algoScene.fixtures` in `puzzle-scenes.js` (insert after `roundedK: (values) => Math.round(values.k),`):
...
Run: `node p5sim/tf2_walkthrough/puzzle-tests.js` → `77/77 puzzle tests passed`. Headless render of the algo lab (54 algorithm builds) and all scenes; a real worker Check on string-reorder, knight-moves-grid, and grid-path-description.

```bash
git add p5sim/tf2_walkthrough/algo/01-introductory.js p5sim/tf2_walkthrough/puzzle-scenes.js p5sim/tf2_walkthrough/puzzle-tests.js
git commit -m "feat: complete the Introductory Problems section of the Algo Lab
