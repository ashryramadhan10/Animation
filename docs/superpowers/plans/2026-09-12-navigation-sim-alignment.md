...
- `broadcastVisionCorrectedTF` 1299-1400 (markers omitted): raw distance, lateral filter, y intercept `= |b|>1e-6 ? -c/b : 0`, projection, x offset along heading, `buildVisionCorrectionTransform`, `storeVisionCorrectionState`.
...
| `correction` | centerline, FCU, projection point, x-offset step, corrected pose, correction vector | raw distance, filtered distance, y intercept, x offset, corrected pose |
