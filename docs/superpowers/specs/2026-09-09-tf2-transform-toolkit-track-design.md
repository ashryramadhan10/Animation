...
33. `invert-homogeneous` — `invertHomogeneous(matrix) → 3×3` using `Rᵀ` and `-Rᵀ t`. Depends on `mat-mul-3`. Scene `matrix` view `inverse` (frame handle; `M · yours` must land on identity). Reference `icp_matching.py`. Diagnoses: `negated-only`, `transpose-whole-matrix`.
...
39. `invert-se3` — `invertSE3(aFromB) → bFromA` with conjugate rotation and `rotate(conj, −t)`. Depends on `rotate-by-quaternion`. Scene `se3` view `inverse` (`aFromB ∘ yours` must land on the world axes). Reference tf2 `Transform::inverse`. Diagnoses: `negated-only`, `translation-not-rotated`.
